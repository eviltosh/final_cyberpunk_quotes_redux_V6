...
def render_matplotlib_cyberpunk_chart(hist: pd.DataFrame, ticker: str, bg_image):
    ...
    if bg_image is not None:
        try:
            ax.imshow(bg_image, extent=[hist.index.min(), hist.index.max(), hist['Close'].min(), hist['Close'].max()], aspect='auto', alpha=1.0, zorder=0)
        except Exception:
            pass
...
def render_plotly_fallback(hist: pd.DataFrame, ticker: str):
    ...
    fig.update_layout(template='plotly_dark', margin=dict(l=0, r=0, t=30, b=0), height=320, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')

    # Keep gridlines + axes visible while making background transparent
    fig.update_xaxes(showgrid=True, gridcolor='rgba(255,255,255,0.3)', zeroline=False, showline=True, linecolor='rgba(255,255,255,0.6)', ticks='outside', tickformat='%m/%d/%y')
    fig.update_yaxes(showgrid=True, gridcolor='rgba(255,255,255,0.3)', zeroline=False, showline=True, linecolor='rgba(255,255,255,0.6)')
    ...
    try:
        ...
        if news:
            for article in news[:5]:
                ...
                safe_markdown(f"<div class='news-card'><a href='{article.get('url')}' target='_blank'><b>{article.get('headline')}</b></a><br><small>{article.get('source','Unknown')} | {t_str}</small></div>")
        else:
            if finnhub_api:
